...
run() {
  container=$1; image=$2; tag=$3; aspnetcore_env=$4;

  rm $container
  if [[ $(use_proxy) = true ]]; then
    echo Running with proxy...
  	docker run --name $container -p 5001:5001 \
    ...
    echo Running without proxy...
    docker run -d --name $container -p 5001:5001 \ 
...
handle_command() {
  validate_dockersetup $(running_locally) $MACHINE_NAME
  case "$1" in
    build)
      build $2 $3 $4
      ;;
