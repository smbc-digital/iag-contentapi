...
public class GroupRepository : BaseRepository, IGroupRepository
{
    ...
    public async Task<HttpResponse> GetGroup(string slug, bool onlyActive)
    {
        ...
        ContentfulGroup entry = onlyActive
            ? entries.FirstOrDefault(group => _dateComparer.DateNowIsNotBetweenHiddenRange(group.DateHiddenFrom, group.DateHiddenTo))
        ...
        if (group.CategoriesReference is not null && group.CategoriesReference is not null && group.CategoriesReference.Any() && group.SubCategories.Any())
    ...
    public async Task<List<Group>> GetLinkedGroups(Group group)
    {
        ...
        IEnumerable<Group> linkedGroups = groupList.Where(group => group.CategoriesReference.Any(category => string.IsNullOrEmpty(group.CategoriesReference[0].Slug) || category.Slug.ToLower().Equals(group.CategoriesReference[0].Slug.ToLower()))
                                   && group.SubCategories.Any(category => string.IsNullOrEmpty(group.SubCategories[0].Slug) || category.Slug.ToLower().Equals(group.SubCategories[0].Slug.ToLower()))
    ...
    public async Task<HttpResponse> GetGroupResults(GroupSearch groupSearch, string slugs)
    {
        ...
        if (groupSearch.Longitude.Equals(0) && groupSearch.Latitude.Equals(0))
            builder = builder.FieldEquals("fields.mapPosition[near]", $"{groupSearch.Latitude},{groupSearch.Longitude}{(groupSearch.Location.ToLower().Equals(Constants.Groups.Location) ? ",10" : ",3.2")}");
        ...
        if (groupSearch.Location.ToLower().Equals(Constants.Groups.Location))
        {
            ...
            groupsWithNoCoordinates = noCoordinatesEntries.Select(group => _groupFactory.ToModel(group))
                ...
                .Where(group => group.CategoriesReference.Any(category =>
                    string.IsNullOrEmpty(groupSearch.Category) || category.Slug.ToLower().Equals(groupSearch.Category.ToLower())))
                .Where(group => string.IsNullOrWhiteSpace(groupSearch.Tags) || group.Tags.Contains(groupSearch.Tags.ToLower()))
                .Where(group => _dateComparer.DateNowIsNotBetweenHiddenRange(group.DateHiddenFrom, group.DateHiddenTo))
                .Where(group => string.IsNullOrEmpty(groupSearch.GetInvolved) || (group.Volunteering && groupSearch.GetInvolved.Equals("yes")))
                .Where(group => string.IsNullOrEmpty(groupSearch.Organisation) ||
                            (group.Organisation is not null && group.Organisation.Slug.Equals(groupSearch.Organisation)))
        ...
        List<Group> groups =
             entries.Select(group => _groupFactory.ToModel(group))
                    .Where(group => group.CategoriesReference.Any(category => string.IsNullOrEmpty(groupSearch.Category) || category.Slug.ToLower().Equals(groupSearch.Category.ToLower())))
                    .Where(group => string.IsNullOrWhiteSpace(groupSearch.Tags) || group.Tags.Contains(groupSearch.Tags.ToLower()))
                    .Where(group => _dateComparer.DateNowIsNotBetweenHiddenRange(group.DateHiddenFrom, group.DateHiddenTo))
                    .Where(group => string.IsNullOrEmpty(groupSearch.GetInvolved) || (group.Volunteering && groupSearch.GetInvolved.Equals("yes")))
                    .Where(group => string.IsNullOrEmpty(groupSearch.Organisation) || (group.Organisation is not null && group.Organisation.Slug.Equals(groupSearch.Organisation)))
                    .Where(group => !subCategoriesList.Any() || group.SubCategories.Any(category => subCategoriesList.Contains(category.Slug)))
                 .Where(_ => !_.MapPosition.Lat.Equals(0) && !_.MapPosition.Lon.Equals(0))
                    .ToList();
