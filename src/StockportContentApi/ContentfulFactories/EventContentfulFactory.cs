...
namespace StockportContentApi.ContentfulFactories
{
    public class EventContentfulFactory : IContentfulFactory<ContentfulEvent, Event>
    {
        public Event ToModel(ContentfulEvent entry)
        {
            ...
            return new Event(entry.Title, entry.Slug, entry.Teaser, entry.Image.File.Url, entry.Description, entry.Fee, entry.Location, entry.SubmittedBy, entry.Longitude,
                entry.Latitude, entry.Featured, entry.EventDate, entry.StartTime, entry.EndTime, entry.Occurences, entry.Frequency, entry.Breadcrumbs,
