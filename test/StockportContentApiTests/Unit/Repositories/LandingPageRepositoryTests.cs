...
public class LandingPageRepositoryTests
{
    ...
    public async Task GetLandingPage_GetsEventsFromCategory_WhenCategoryEventListIsNotEmpty()
    {
        ...
        List<Event> events = new() { new EventBuilder().Build(), new EventBuilder().Build(), new EventBuilder().Build() };
    ...
    public async Task GetProfile_ReturnsProfile_WhenProfileExists()
    {
        ...
        _contentfulClient.Setup(client => client.GetEntries(It.IsAny<QueryBuilder<ContentfulProfile>>(), It.IsAny<CancellationToken>()))
